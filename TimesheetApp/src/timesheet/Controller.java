...
public class Controller {
	...
	public void processStopAction(String[] actionParts) {
		...
		// Your code here
		this.timesheet.stop(timesheet.get(id));
	}
	...
	public void processListAction(String[] actionParts) {
		...
		if (actionParts.length == 1) {
			this.consoleUtils.printList(this.timesheet.list(false));
			return;
		}
		// Your code here
		if (actionParts.length == 2) {
			if (actionParts[1].equals("-a"))
				this.consoleUtils.printList(this.timesheet.list(true));
			else
				this.consoleUtils.printList(this.timesheet.list(actionParts[1]));
			return;
		}
		...
		if (actionParts.length == 3) {
			...
			if (actionParts[1].equals("-a"))
				this.consoleUtils.printList(this.timesheet.list(true, actionParts[2] ));
			else if (actionParts[2].equals("-a"))
				this.consoleUtils.printList(this.timesheet.list(false, actionParts[1]));
